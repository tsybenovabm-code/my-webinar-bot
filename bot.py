...
from config import BOT_TOKEN, ADMIN_IDS, COURSE_INVITE_LINK, COURSE_CHANNEL_NAME, CHANNEL_USERNAME
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await give_access(update.message, user, context)


async def give_access(message, user, context):
   ...
   if is_new:
        ...
        "Также напоминаем, что 17 мая в 17:30 мы проведем вебинар по A/B-тестам, напоминанием и ссылку пришлем сюда, не заубдь включить уведомления.\n\n"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "check_subscription":
        ...
        if is_subscribed:
            ...
            await give_access(query.message, user, context)
        else:
            await query.answer(
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Например:\n"
            "/broadcast Вебинар сегодня в 19:00! Ссылка: [ссылка]"
        )
        return
